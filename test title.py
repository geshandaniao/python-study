...
res = requests.get('https://bj.xiaozhu.com/fangzi/135545608403.html', headers=headers)  # 网站为小猪短租网北京地区网址，get方法加入请求头
...
for title in titles:      #此时prices为列表，需循环遍历
    print(title.get_text())      #通过get_text()方法获取文字信息

addresses = soup.select('div.con_l > div.pho_info > p > span')
for address in addresses:      #此时prices为列表，需循环遍历
    print(address.get_text())      #通过get_text()方法获取文字信息
